...
---
Guide to running tests across multiple regions using the orchestrator.
...
Run has been submitted but not started.

### running

Run is in progress (one or more jobs executing).
...
The orchestrator groups runners into jobs:

1. **By URL**: Runners with the same URL are grouped
2. **By Region**: In remote mode, runners are grouped by region
3. **One Job Per URL-Region**: Each unique URL-region combination gets one job
...
```json
{
  "runners": [
    ...
    { "name": "test1", "region": "eu-west-1", "input": { "url": "https://example.com" } }
  ],
  "mode": "remote"
}
```
...
Creates 2 jobs:
- Job 1: `us-east-1` → runs `test1` and `test2`
- Job 2: `eu-west-1` → runs `test1`
...
- Run status becomes `"failed"` if any job fails

### Runner Errors

If a runner errors:
- Runner result has `status: "error"`
- Job continues with other runners
- Job status becomes `"failed"` if all runners error

### Network Errors

If a remote runner endpoint is unreachable:
- Job status becomes `"failed"`
...
Tags are included in run metadata for filtering and organization.

## Nitro Integration

Use the Nitro orchestrator module:
...
## Example: Multi-Region Testing

Test a website across multiple regions:
...
const runRequest = {
  ...
  mode: "remote",
  concurrency: 3  // Run 3 regions in parallel
};

const { runId } = await submitRun(runRequest);
const summary = await waitForCompletion(runId);
